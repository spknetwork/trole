...
then
    ...
    if test -f "Caddyfile.template"; then
        sed -e "s/{{DOMAIN}}/${DOMAIN}/g" -e "s/{{API_PORT}}/${API_PORT}/g" Caddyfile.template | sudo tee -a $CADDY_FILE
        sudo systemctl restart caddy
    else
        echo -e "${RED}Caddyfile.template not found! Please ensure it exists in the current directory.${NC}"
        exit
    fi
else
    echo Caddy config exists: Ensure it matches the template
    echo -e "${YELLOW}To update Caddy config, remove existing config from ${CADDY_FILE} and run script again${NC}"
    echo -e "${YELLOW}Or manually update using Caddyfile.template as reference${NC}"
fi
